...
class artikel with ChangeNotifier {
  ...
  Article selectById(String id) {
    return _allarticle.firstWhere((element) => element.id == id, orElse: () => null as Article);
  }
    ...
    try {
      await http
        ...
        if (response.statusCode == 200) {
          ...
          _allarticle.add(Article(id: id["name"], image: image, title: title, description: description, comments: comment));
  ...
  Future<void> getAllArticle() async {
    ...
    if (response.statusCode == 200) {
      ...
      article.forEach((key, value) {
        _allarticle.add(Article(id: key, title: value["title"], description: value["description"], image: value["image"], comments: value["comment"]));
      });
      notifyListeners();
    }
  }
